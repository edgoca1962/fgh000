...
class ComedorModel
{
   ...
   public function __construct()
   {
      ...
      // add_action('save_post', [$this, 'fghegc_save_mapa_coordenadas']);

      add_action('wp_ajax_agregar_comedor', [$this, 'agregar_comedor']);
      add_action('wp_ajax_editar_comedor', [$this, 'editar_comedor']);
      add_action('wp_ajax_eliminar_comedor', [$this, 'eliminar_comedor']);
      $this->set_paginas();
      $this->set_roles();
   }
   ...
   public function fghegc_set_coordenadas($post)
   {
      $latitude = get_post_meta($post->ID, '_map_latitude', true);
      $longitude = get_post_meta($post->ID, '_map_longitude', true);

      echo '<label for="map_latitude">Latitude:</label>';
      echo '<input type="text" name="map_latitude" id="map_latitude" value="' . esc_attr($latitude) . '" />';

      echo '<label for="map_longitude">Longitude:</label>';
      echo '<input type="text" name="map_longitude" id="map_longitude" value="' . esc_attr($longitude) . '" />';
   }
   public function fghegc_save_mapa_coordenadas($post_id)
   {
      if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) return;

      if (isset($_POST['map_latitude'])) {
         update_post_meta($post_id, '_map_latitude', sanitize_text_field($_POST['map_latitude']));
      }

      if (isset($_POST['map_longitude'])) {
         update_post_meta($post_id, '_map_longitude', sanitize_text_field($_POST['map_longitude']));
      }
   }
   ...
   private function set_roles()
   {
      ...
      add_role('comedores', 'Comedores', get_role('subscriber')->capabilities);
   }
   private function set_paginas()
   {
      $paginas = [
         ...
         [
            ...
            'titulo' => 'Mantenimiento de Comedores',
         ],
      ];
   ...
   public function agregar_comedor()
   {
   }
   public function editar_comedor()
   {
   }
   public function eliminar_comedor()
   {
   }
}
